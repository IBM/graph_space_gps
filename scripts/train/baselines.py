...
def train_loop(args, train_inputs, test_inputs, model, likelihood, orig_std):
    ...
    for i in range(args.train_iter):
        ...
        loss.backward()
        norm_params = {name: p.grad.detach()for name, p in model.named_parameters()}
        ...
        if i % args.print_every == 0:
            ...
            if args.kernel == "rbf":
                kappa = model.covar_module.base_kernel.lengthscale.item()
                print_msg += f" | Kappa: {np.round(kappa, 4)}"

            # if hasattr(model.covar_module, 'kappa'):
            #     kappa = model.covar_module.kappa.item()
            #     print_msg += f" | Kappa: {np.round(kappa, 4)}"
            
            # if hasattr(model.covar_module, 'sigma2'):
            #     sigma2 = model.covar_module.sigma2.item()
            #     print_msg += f" | Sigma2: {np.round(sigma2, 4)}"
            
            # if hasattr(model.covar_module, "nu"):
            #     nu = model.covar_module.nu.item()
        ...
        if i % args.eval_every == 0:
            ...
            with torch.no_grad(), gpytorch.settings.fast_pred_var():
                ...
                lik_test = np.round(pred_y.log_prob(y_test).item(), 4)

                print(f"Test Metrics || Likelihood {lik_test} | L2 Error: {l2_error} | L2 Norm Naive: {l2_error_base}")
...
def parse_args():
    ...
    parser.add_argument('--kernel', default="graph", 
                        help="Which kernel to use. Choices: []")
